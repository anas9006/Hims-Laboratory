...
const LabTestsSpecimens = () => {
  ...
  const handleSave = () => {
    if (newSpecimenName.trim()) {
      ...
      const newSrNo = specimens.length + 1;
      setSpecimens([...specimens, { 
        id: newId, 
        srNo: newSrNo, 
        specimenName: newSpecimenName.trim() 
      }]);
      setNewSpecimenName("");
    }
  };
  ...
  const handleSaveEdit = () => {
    if (editValue.trim() && editingId) {
      setSpecimens(specimens.map(s => 
        s.id === editingId ? { ...s, specimenName: editValue.trim() } : s
      ));
      setEditingId(null);
      setEditValue("");
    }
  ...
  const handleDelete = (id) => {
    ...
    const filteredSpecimens = specimens.filter(s => s.id !== id);
    // Update sr numbers sequentially
    const updatedSpecimens = filteredSpecimens.map((specimen, index) => ({
      ...specimen,
      srNo: index + 1
    }));
  ...
  const filtered = specimens.filter(s => 
    s.specimenName.toLowerCase().includes(searchTerm.toLowerCase())
  );
